...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        arguments = expr.second.map(lambda expr: scheme_eval(expr, env))
        return scheme_apply(scheme_eval(expr.first,env), arguments,env)
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    test = scheme_eval(expressions.first,env, expressions.second is nil)
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            arguments = expr.second.map(lambda expr: scheme_eval(expr, env))
            result = scheme_apply(scheme_eval(expr.first,env), arguments,env)
