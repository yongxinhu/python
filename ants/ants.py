...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant and isinstance (insect,QueenAnt) and insect.real_Queen:
            return
        elif insect.is_ant:
...
class Ant(Insect):
    ...
    def can_contain(self, ant):
        if self.container and ant.container == False and self.ant == None:
            return True
        return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        original = self.place
        n = 0
        while original!=hive and (original.bees == [] or n < self.min_range or n > self.max_range):
            original = original.entrance
            n=n+1
        if original == hive:
            return None
        return random_or_none(original.bees)
        # END Problem 3B
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower can be palced on Water but otherwise identical to ThrowerAnt"""
    name = 'Scuba'
    food_cost = 6
    watersafe = True
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.real_Queen == True:
           ...
           while original.exit != None:
                if original.exit.ant and (original.exit.ant not in QueenAnt.list_doubled or (isinstance(original.exit.ant,BodyguardAnt) and original.exit.ant.ant)):
                    if isinstance(original.exit.ant,BodyguardAnt) and original.exit.ant.ant:
                        if original.exit.ant not in QueenAnt.list_doubled:
                            original.exit.ant.damage = 2 * original.exit.ant.damage
                            QueenAnt.list_doubled.append(original.exit.ant)
                        if original.exit.ant.ant in QueenAnt.list_doubled:
                            pass
                        else:
                            original.exit.ant.ant.damage = 2 * original.exit.ant.ant.damage
                            QueenAnt.list_doubled.append(original.exit.ant.ant)
                    else:  
                        original.exit.ant.damage = 2*original.exit.ant.damage
                        QueenAnt.list_doubled.append(original.exit.ant)
                original = original.exit
        else:
    ...
    def reduce_armor(self, amount):
        ...
        if self.real_Queen == True and self.armor <= 0:
            bees_win()
...
def apply_effect(effect, bee, duration):
    ...
    bee.action = effects(effect(bee.action),bee.action)
    # END Problem EC
